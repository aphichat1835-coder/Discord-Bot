...
| F-003 | User-authored messages could create mentions | Utility send paths use `allowedMentions: { parse: [] }` | Utility command tests inspect outbound payloads | Closed — automated |
| F-004 | Voice token owner mismatch could continue after logging a warning | Voice lifecycle performs owner checks before and after login | `discord/tests/voiceLifecycleBehavior.test.js` | Closed — automated |
| F-005 | Concurrent Voice creation could create duplicate clients or delete the winner | Token/guild singleflight, generation ownership and ownership-safe cleanup | Voice concurrency tests | Closed — automated |
...
| F-011 | Global environment flags could override per-Guild privacy policy | Guild policy controls token/raw-IP writes and removes disabled fields | Privacy and recovery tests | Closed — automated |
...
| F-017 | Sensitive Token/IP access lacked one central audit contract | Sensitive access service records intent/result and reveals one value at a time; manual reason is intentionally replaced by an automatic Dashboard audit reason per owner instruction | Sensitive-access and owner-reveal tests | Closed — automated |
...
| F-021 | Internal-event filtering and metadata handling could omit matches or leak sensitive values | Sanitiser and filter-before-result-limit behaviour | Internal-event and static-analysis tests | Closed — automated |
