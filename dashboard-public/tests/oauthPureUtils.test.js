...
const { normalizeAction, clampNumber } = require('../utils/verifyMode');
...
describe('compactUserGuild', () => {
    ...
    test('truncates features to 50 items max', () => {
...
describe('compactMemberInfo', () => {
    ...
    test('truncates roles to 80 items max', () => {
        const roles = Array.from({ length: 100 }, (_, i) => `role_${i}`);
        const result = compactMemberInfo({ roles });
        expect(result.roles).toHaveLength(80);
        expect(result.roleCount).toBe(100); // roleCount uses original length
    });
