...
const slashCommandsData = [
        ...
        options: [
            {
                type: 6,
                name: "member",
                description: "สมาชิกที่ต้องการดูข้อมูล",
                required: false,
            },
        ],
    },
    {
        name: "clear",
        description: "ลบข้อความในช่องปัจจุบัน",
        options: [
            {
                type: 4,
                name: "amount",
        ...
        options: [
            {
                type: 3,
                name: "userid",
                description: "User ID ที่ต้องการปลดแบน",
                required: true,
            },
        ],
    },
        ...
        options: [
            {
                type: 6,
                name: "member",
                description: "สมาชิกที่ต้องการระงับ",
                required: true,
            },
            {
                type: 4,
...
function getPanelEmbed() {
    ...
    let sessionList = sessions.length
        ? sessions
              .map(
                  (s) =>
                      `  ∙ Token: ****${s.tokenTail}  │  Server: ${s.serverName || s.serverId}`,
              )
              .join("\n")
        : "  — ไม่มีเซสชันที่ทำงานอยู่ในขณะนี้";

    if (sessionList.length > 850)
        sessionList =
            sessionList.slice(0, 850) +
    ...
    const embed = new MessageEmbed()
        ...
        .addFields({
            name: "📋  รายการเซสชันที่ใช้งานอยู่",
            value: "```yaml\n" + sessionList + "\n```",
            inline: false,
        })
        .setFooter({
            text: `⏱ อัปเดตล่าสุด: ${new Date().toLocaleTimeString("th-TH")}  │  Enterprise Edition`,
        })
        .setTimestamp();
...
function getHelpPages() {
    const footer = {
        text: "Enterprise System  │  เฉพาะผู้ดูแลระบบที่ได้รับอนุญาต",
    };
    return [
        new MessageEmbed()
        ...
        new MessageEmbed()
            ...
            .addFields(
                ...
                },
                {
                    ...
                    value: "> **⚡ เริ่มเซสชัน** กรอก Token เพื่อเริ่ม\n> **📡 สถานะ** ตรวจสอบเซสชัน\n> **⏹ หยุดรายการ** หยุดเฉพาะรายการ\n> **🛑 หยุดทั้งหมด** ปิดทุกเซสชัน",
                    inline: false,
                },
            )
            .setFooter(footer),
    ...
    try {
        if (logWebhook) {
            ...
            const ch = guild.channels.cache.find(
                (c) =>
                    c.name === config.channels.logName &&
                    c.type === "GUILD_TEXT",
            );
    ...
    try {
        switch (cmd) {
            case "clear": {
                ...
                const deleted = await msg.channel
                    .bulkDelete(amount, true)
                    .catch(() => null);
                const m = await msg.channel.send(
                    `> 🗑️  ลบข้อความสำเร็จ ${deleted?.size || amount} รายการ`,
                );
            ...
            case "announce":
                if (!args.length)
                    return msg.reply("> ⛔  กรุณาระบุเนื้อหาประกาศ");
                await msg.delete().catch(() => {});
                await msg.channel.send({
                    embeds: [
                        new MessageEmbed()
                            ...
                            .setFooter({ text: `ประกาศโดย ${msg.author.tag}` })
                            .setTimestamp(),
                    ],
                });
                break;
            ...
            case "kick": {
                if (!target)
                    return msg.reply("> ⛔  กรุณาระบุสมาชิกด้วย @mention");
            ...
            case "timeout": {
                ...
                if (!target || !mins || mins < 1)
                    return msg.reply("> ⛔  กรุณาระบุสมาชิกและจำนวนนาที");
                const reason = args.slice(2).join(" ") || "ไม่ระบุเหตุผล";
                await target.timeout(mins * 60000);
                await msg.reply(
                    `> ⏳  ระงับสิทธิ์ \`${target.user.tag}\` ${mins} นาทีแล้ว`,
                );
                await sendLog(
                    msg.guild,
                    buildModerationEmbed(
            ...
            case "setup-log":
                if (
                    msg.guild.channels.cache.find(
                        (c) => c.name === config.channels.logName,
                    )
                )
                    return msg.reply("> ❌  มีห้อง Log อยู่แล้ว");
                await msg.guild.channels.create(config.channels.logName, {
                    type: "GUILD_TEXT",
                });
...
async function handleInteraction(interaction) {
    ...
    const hasAccess =
        isAdmin || interaction.member.roles.cache.has(config.roles.user);
            ...
            switch (cmd) {
                ...
                case "clear": {
                    ...
                    return interaction.editReply({
                        content: `> 🗑️  ลบข้อความสำเร็จ ${deleted?.size || amount} รายการ`,
                    });
                }
                case "say": {
                ...
                case "kick": {
                    ...
                    await interaction.reply({
                        content: `> ⚔️  เตะสมาชิก \`${member.user.tag}\` แล้ว`,
                        ephemeral: true,
                    });
                    await sendLog(
                        interaction.guild,
                ...
                case "ban": {
                    ...
                    await interaction.reply({
                        content: `> 🔨  แบนสมาชิก \`${member.user.tag}\` แล้ว`,
                        ephemeral: true,
                    });
                    await sendLog(
                        interaction.guild,
                ...
                case "timeout": {
                    ...
                    await interaction.reply({
                        content: `> ⏳  ระงับสิทธิ์ \`${member.user.tag}\` ${mins} นาทีแล้ว`,
                        ephemeral: true,
                    });
                    await sendLog(
                        interaction.guild,
    ...
    if (interaction.isButton()) {
        ...
        if (
            PANEL_BTNS.includes(interaction.customId) &&
            !panelMessages.has(interaction.channelId)
        ) {
        ...
        if (interaction.customId === "btn_start") {
            if (
                ...
                return interaction.reply({
                    content: `> ⛔  ถึงขีดจำกัด ${config.limits.maxSessions} เซสชัน`,
                    ephemeral: true,
                });
            const modal = new Modal()
                .setCustomId("setup_modal")
                .setTitle("⚙️ ตั้งค่าข้อมูลการออนช่องเสียง");
            modal.addComponents(
        ...
        if (interaction.customId === "btn_status") {
            ...
            let sessionList = sessions
                .map(
                    (s) =>
                        `  ∙ Token: ****${s.tokenTail}  │  Server: ${s.serverName || s.serverId}\n    Runtime: ${formatUptime(Date.now() - s.startedAt)}`,
                )
                .join("\n\n");
            const header = `> **📡  สถานะระบบออนช่องเสียง  ·  ${sessions.length} / ${config.limits.maxSessions} เซสชัน**\n\`\`\`yaml\n`;
            if (sessionList.length > 1800)
                sessionList =
                    sessionList.slice(0, 1800) + "\n\n  … (มีรายการเพิ่มเติม)";
            return interaction.reply({
                content: header + sessionList + "\n```",
                ephemeral: true,
            });
        }
        ...
        if (interaction.customId === "btn_stop") {
            ...
            return interaction.editReply({
                content: `> 🛑  หยุดการทำงานทั้งหมด ${count} เซสชันสำเร็จ`,
            });
        }
            ...
            const menu = new MessageSelectMenu()
                ...
                .addOptions(
                    sessions
                        ...
                        .map((s) => ({
                            label: safeSlice(
                                `****${s.tokenTail}  ·  ${s.serverName || s.serverId}`,
                                100,
                            ),
                            value: s.sessionId,
                        })),
    ...
    if (interaction.isSelectMenu() && interaction.customId === "select_stop") {
        ...
        const label = session
            ? safeSlice(
                  `****${session.tokenTail}  ·  ${session.serverName || session.serverId}`,
                  100,
              )
            : sessionId;
        ...
        try {
            ...
            await sendLog(
                ...
                new MessageEmbed()
                    ...
                    .setDescription("> **สถานะ**: กำลังเชื่อมต่อห้องเสียง")
                    .addFields(
                        {
                            name: "👤  ผู้ดำเนินการ",
                            value: `\`\`\`yaml\nUser: ${interaction.user.tag}\nID: ${interaction.user.id}\n\`\`\``,
            ...
            await interaction.editReply({
                content:
                    "> ⚡  เริ่มเซสชันสำเร็จ — กำลังเชื่อมต่อระบบออนช่องเสียง",
            });
        } catch (err) {
            const errMsg =
                {
