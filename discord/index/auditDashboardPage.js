...
function csrfHeaders(){const token=decodeURIComponent(readCookie('__da_csrf'));return token?{'Content-Type':'application/json','x-csrf-token':token}:{'Content-Type':'application/json'}}
function guild(){return document.getElementById('guildId').value.trim()}
function readNumberInput(id,min,max){const raw=document.getElementById(id).value.trim();const n=Number(raw);if(!raw||!Number.isFinite(n)||n<min||n>max){throw new Error(id+' ต้องเป็นตัวเลข '+min+'-'+max)}return n}
...
async function saveSettings(){let body;try{body={guildId:guild(),messageCreateEnabled:document.getElementById('messageCreateEnabled').checked,reconcilerEnabled:document.getElementById('reconcilerEnabled').checked,retentionDays:readNumberInput('retentionDays',0,3650),reconcilerLimit:readNumberInput('reconcilerLimit',1,50)}}catch(e){document.getElementById('settingsStatus').textContent=e.message;return}const r=await fetch('/api/audit/settings',{method:'POST',headers:csrfHeaders(),body:JSON.stringify(body)});const j=await r.json();document.getElementById('settingsStatus').textContent=j.success?'saved':j.error}
</script>
</body>
</html>`;
}
